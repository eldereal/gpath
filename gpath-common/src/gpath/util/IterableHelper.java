...
public final class IterableHelper {
    ...
    public static Iterable<Long> range(final long start, final long end){
    	return range(start, end, 1);
    }
    
    public static Iterable<Long> range(final long start, final long end, final long step){
    	if(step<=0) throw new ArithmeticException("step cannot be less than zero");
    	return new SimpleCollection<Long>() {
			...
			public boolean contains(Object obj) {
				if(obj instanceof Integer){
					...
					if(step>0){
						...
						return v>end && v<=start && (start - v)%(-step)==0;
					}
				}else{
					return false;
				}
	...
	public static <T> Iterable<T> concat(final Iterable<? extends T> in1,final Iterable<? extends T> in2) {
    	return concat(new Iterable[]{in1,in2});
    }
